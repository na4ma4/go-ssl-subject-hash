...
func workingExample() {
	testArray := [][]byte{
		...
		{
			0x31, 0x19, 0x30, 0x17, 0x06, 0x03, 0x55, 0x04, 0x03, 0x0C, 0x10, 0x61, 0x6D, 0x61, 0x7A, 0x6F, 0x6E,
			0x20, 0x72, 0x6F, 0x6F, 0x74, 0x20, 0x63, 0x61, 0x20, 0x31,
		},
	}
	...
	for i, v := range testArray {
		...
		_, err := terms.Write(v)
		checkErr(err)
		h := sha1.Sum(v) //nolint:gosec // openssl compat
		n := truncatedHash(h, 4)
		log.Printf("Sum[%d]: (%x) [%x]", i, h, n)
	}

	// finalEncForw := make([]byte, finalLen)
	h := sha1.Sum(terms.Bytes()) //nolint:gosec // openssl compat
	n := truncatedHash(h, 4)
